...
import com.lorettax.analy.maps.UserNetworkMap;
import com.lorettax.analy.reduce.UserNetworkSink;
import com.lorettax.analy.reduces.UserNetworkReduce;
...
//		}
        ...
        DataStream<NetworkUser> networkmap = input.map(new UserNetworkMap());

        DataStream<NetworkUser> networkreduce = networkmap.
                keyBy("groupbyfield").
                timeWindowAll(Time.seconds(2)).
                reduce(new UserNetworkReduce());

        networkreduce.addSink(new UserNetworkSink());

        env.execute("networkUser analy");
    }
}
