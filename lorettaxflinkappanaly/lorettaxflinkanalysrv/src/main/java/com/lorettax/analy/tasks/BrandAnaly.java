...
import com.lorettax.analy.maps.BrandMap;
import com.lorettax.analy.maps.StartupMap;
import com.lorettax.analy.reduces.BrandReduce;
import com.lorettax.analy.reduces.StartupReduce;
import com.lorettax.analy.sinks.BrandSink;
...
//		}
        ...
        DataStream<BrandUser> brandmap = input.map(new BrandMap());

        DataStream<BrandUser> startupreduce = brandmap
                .keyBy("groupbyfield")
                .timeWindowAll(Time.seconds(2))
                .reduce(new BrandReduce());

        startupreduce.addSink(new BrandSink());

        env.execute("branduser analy");
    }
}
