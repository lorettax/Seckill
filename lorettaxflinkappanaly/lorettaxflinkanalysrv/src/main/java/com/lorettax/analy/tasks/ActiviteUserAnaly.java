...
import com.lorettax.analy.maps.ActivitieUserMap;
import com.lorettax.analy.maps.StartupMap;
import com.lorettax.analy.reduces.ActivitieUserReduce;
import com.lorettax.analy.reduces.StartupReduce;
import com.lorettax.analy.sinks.ActivitieUserSink;
...
public class ActiviteUserAnaly {
...
//		}
        ...
        DataStream<ActivitieUser> activitieUsermap = input.map(new ActivitieUserMap());

        DataStream<ActivitieUser> activitieUserreduce = activitieUsermap
                .keyBy("groupbyfield")
                .timeWindowAll(Time.seconds(2))
                .reduce(new ActivitieUserReduce());

        activitieUserreduce.addSink(new ActivitieUserSink());

        env.execute("activitieUser analy");
    }
