...
//		}
        ...
        DataStream<Startup> startupmap = input.map(new StartupMap());

        DataStream<Startup> startupreduce = startupmap.keyBy("groupbyfield").timeWindowAll(Time.seconds(2)).reduce(new StartupReduce());

        startupreduce.addSink(new StartupSink());
