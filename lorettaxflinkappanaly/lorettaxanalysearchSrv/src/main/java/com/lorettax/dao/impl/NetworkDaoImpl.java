 ...
 */
@Component
public class NetworkDaoImpl extends BaseMongo implements NetWorkDao {
    ...
    public List<NetworkUser> listNetworkInfoby(String appId, String timeFrom, String timeTo, String timeUnit, String appChannel, String appVersion, String appPlatform) {
        List<NetworkUser> result = new ArrayList<NetworkUser>();
        ...
        if("daily".equals(timeUnit)){
            collection = db.getCollection("networkuserinfodaily");
        }else{
            return result;
        }
        ...
        while(cursor.hasNext()){
            ...
            NetworkUser networkUser = JSONObject.parseObject(jsonString,NetworkUser.class);
            result.add(networkUser);
        }
        return result;
    }
}
